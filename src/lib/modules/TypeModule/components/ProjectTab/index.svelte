...
<ProcessingModal />
<AddProjectModal bind:open={openAddModal} bind:banInitModal={banInitModal} />

<div>
    <Button kind="ghost" on:click={() => (openAddModal = true)}
        >添加新项目</Button
    >
    <Button kind="ghost" on:click={() => fetchProjects(true)}
        >全量更新所有项目</Button
    >
    <Button kind="ghost" on:click={() => fetchProjects(false)}
        >增量更新所有项目</Button
    >
</div>
            ...
            <TabContent>
                <div transition:slide={{ duration: 300 }}>
                    <div style="margin-bottom:10px">
                        <Button
                            on:click={() =>
                                fetchProjects(true, project.project_id)}
                            >全量更新当前项目</Button
                        >
                        <Button
                            on:click={() =>
                                fetchProjects(false, project.project_id)}
                            >增量更新当前项目</Button
                        >
                    </div>
